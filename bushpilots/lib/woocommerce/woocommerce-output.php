...
add_filter( 'woocommerce_enqueue_styles', 'mono_pro_woocommerce_styles' );
/**
 ...
 */
function mono_pro_woocommerce_styles( $enqueue_styles ) {

	$enqueue_styles[ genesis_get_theme_handle() . '-woocommerce-styles' ] = [
		'src'     => get_stylesheet_directory_uri() . '/lib/woocommerce/bushpilots-woocommerce.css',
		'deps'    => '',
...
add_action( 'wp_enqueue_scripts', 'mono_pro_woocommerce_css' );
/**
 ...
 */
function mono_pro_woocommerce_css() {
	...
	$color_link   = get_theme_mod( 'mono_pro_link_color', $appearance['default-colors']['link'] );
	$color_accent = get_theme_mod( 'mono_pro_accent_color', $appearance['default-colors']['accent'] );
	...
	',
		$color_accent,
		mono_pro_color_contrast( $color_accent )
	) : '';
