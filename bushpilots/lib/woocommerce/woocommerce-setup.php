...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'mono_pro_woocommerce_breakpoint' );
/**
 ...
 */
function mono_pro_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'mono_pro_default_products_per_page' );
/**
 ...
 */
function mono_pro_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'mono_pro_woocommerce_pagination' );
/**
 ...
 */
function mono_pro_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'bushpilots-pro' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'bushpilots-pro' ) );

	return $args;

}

add_action( 'after_switch_theme', 'mono_pro_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function mono_pro_woocommerce_image_dimensions_after_theme_setup() {
	...
	mono_pro_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'mono_pro_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function mono_pro_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function mono_pro_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'mono_pro_gallery_image_thumbnail' );
/**
 ...
 */
function mono_pro_gallery_image_thumbnail( $size ) {
